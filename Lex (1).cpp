...
//			MkExec(ReceiverMK.back(), Load.Copy());
		...
		{	string str = Load.toStr()+" ";
			str += EOL; // Добавить символы конца строки
		...
		//Enver//
		case 101: // Добавление нового обозначения комментария
		{
			string addComment = Load.toStr();
			string first = ""; //переменная для выделения открывающего комментария
			string second = ""; //переменная для выделения закрывающего комментария
			int index; //индекс разделения открывающего и закрывающего комментариев в поданной строке
	
			for (index = 0; index < addComment.length() and addComment[index] != ' '; index++) //поиск открывающего комментария
			{
				first += addComment[index];
			}
			for (index; index < addComment.length() and addComment[index] == ' '; index++); //пропуск пробелов между строками
			second = addComment.substr(index); //добавление оставшейся части строки к закрывающему комментарию

			if (find(Comments.begin(), Comments.end(), pair(first, second)) == Comments.end())
				Comments.push_back({first, second}); //добавление открывающего и закрывающего комментариев к вектору пар, если такой пары не было

			break;
		}
		//Enver//
		case 102: // Удаление обозначения комментария
		{
			string delComment = Load.toStr();
			string first = ""; //переменная для выделения открывающего комментария
			string second = ""; //переменная для выделения закрывающего комментария
			int index; //индекс разделения открывающего и закрывающего комментариев в поданной строке

			for (index = 0; index < delComment.length() and delComment[index] != ' '; index++) //поиск открывающего комментария
			{
				first += delComment[index];
			}
			for (index; index < delComment.length() and delComment[index] == ' '; index++); //пропуск пробелов между строками
			second = delComment.substr(index); //добавление оставшейся части строки к закрывающему комментарию

			Comments.erase(find(Comments.begin(), Comments.end(), pair(first, second))); //удаление открывающего и закрывающего комментариев в векторе пар, если такая пара есть
