...
#  endif
        ...
        const std::map<std::string, std::string>& keywords = {})
    {
        ...
        return bar(x, y, ec, ls, lw, width, keywords);
    }


    template<typename Numeric>
    bool barh(const std::vector<Numeric>& x, const std::vector<Numeric>& y, std::string ec = "black", std::string ls = "-", double lw = 1.0, double height = 1.0, const std::map<std::string, std::string>& keywords = { }) {
        PyObject* xarray = detail::get_array(x);
        PyObject* yarray = detail::get_array(y);
        ...
        PyDict_SetItemString(kwargs, "ls", PyString_FromString(ls.c_str()));
        PyDict_SetItemString(kwargs, "lw", PyFloat_FromDouble(lw));
        PyDict_SetItemString(kwargs, "height", PyFloat_FromDouble(height));
