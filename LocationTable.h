...
private:
	...
	long int ind = -1; // »ндекс адреса в таблице адресов (при -1 используетс€ вектора индексов)
...
public:
	...
	long int Index(void* Point); // ¬озвращаетс€ индекс найденного указател€ (если указатель не найден, возвращаетс€ -1)
	...
	vector<void*>* GetLine(void* Point); // ¬ыдать ссылку на линию списка, где обнаружилось совпадение
	bool Empty(); // ѕроверка на пустоту таблицы
	long int LineCount(); // ¬озвращает количество линий в таблице
	void DelLastLine(long int Count=1); // ”дал€ет Count последних строк таблицы 
