...
private:
	...
	set<long int> FinMk; // Множество индексов МК для потребителя, характерных для финального состояния
	...
	long int ProgLevel = 0; // Уровень скобочной конструкции
	...
	long StopPos = -1; // Позиция останова лексичексого анализа (т.е на каком по счету символе закончился лексический анализ
	string StopMark = "^"; // Маркер обозначения позиции останова лексического анализа
	long ErrCode = 0;//Код ошибки
	// Сообщения к каждому коду ошибки
	map<long, string> ErrList = { {1,"Lexical error"}, {2, "Numdder error"}, {3, "float error"}, {4, "Mnemonic error"}, {5, "String error"}, {6, ""}, {7, ""}, {8, ""} };
	string::iterator i; // Итератор по символам строки
	string str; // Разбираемая строка
public:
	...
	vector<long int> ReceiverMK = { 0 }; // Стек МК для приемников
	long int S = 0; // Номер состояния распознающего автомата
...
//	int MnemoAtr = -2, SeperatAtr = -4, IntAtr = -10, DoubleAtr = -3, BoolAtr = -5, StrAtr = -6, ConstAtr = -13;
	...
	long int ib = 0, SizeBuf = 5; // Текущая позиция выходной ИП выходной лексемы и размер буфера
	ip *LexBuf; // Буфер выходных лексем
	map<long int,MkFu> UnicAtr; // Список специфических атрибутов, по которым идет обработки другими ФУ
	long int LastUnicAtr = 0; // Последний уникальный атрибут (чтобы можно было потом добавить к нему МК и адрес ФУ)
