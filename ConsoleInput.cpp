...
//			std::istream& in == cin;
	...
	case 100: // Input — считать и записать в адрес из VarOutBuf
	{
		getline(cin, inStr);
		Var.Clear();
		...
		if (regex_match(inStr, regular_float)) {
			...
			// +1 для sentinel -1 в конце
			long int* arr = new long int[nums.size() + 1];
			for (int i = 0; i < (int)nums.size(); i++)
				arr[i] = nums[i];
			arr[nums.size()] = -1; // sentinel
			...
			while (getline(ss, token, ',')) {
				...
				if (regex_match(token, regular_float)) {
					vect->push_back({ Cdouble, new double(stod(token)) });
				}
				else if (regex_match(token, regular_int)) {
					vect->push_back({ Cint, new long int(stol(token)) });
				}
				else if (regex_match(token, regular_true)) {
					vect->push_back({ Cbool, new bool(true) });
				}
				else if (regex_match(token, regular_false)) {
